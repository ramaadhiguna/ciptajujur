            ...
            <fieldset>
              ...
              if($row = mysqli_fetch_object($resultPembelian)){
                echo "<div class=box-body>
                  ...
                  <label for=inputNamaKaryawan class=col-sm-2 control-label>Nomor PO</label>
                    <div class=col-sm-10>
                      <input type=text name=noPO value='".$row->id."' disabled autofocus class=form-control>
                    </div>
                  </div>
                  <div class=form-group>
                  <label for=inputNamaKaryawan class=col-sm-2 control-label>Nomor LPB</label>
                    <div class=col-sm-10>
                      <input type=text name=noNota value='".$date.$nomorBaru."' disabled autofocus class=form-control>
                    </div>
                  </div>
                  <div class=form-group>
              ...
              if(($rowKirim->qty-$rowKirim->saldo)>0){
              echo "<div class=box-body>
                      <div class=form-group>
                      ...
                      <div class=form-group>
                        <label for=inputUserKaryawan class=col-sm-2 control-label>Jumlah</label>
                          <div class=col-sm-10>
                            <input type=number name=jumlah[] placeholder = 'Jumlah Order = ".($rowKirim->qty-$rowKirim->saldo)." Meter' max ='".($rowKirim->qty-$rowKirim->saldo)."'required autofocus class=form-control>
                            <input type=hidden name=harga[] value=".$rowKirim->harga.">
                          </div>
                      </div>              
                    </div>";
                  }
              ...
              if(($rowKirimb->panjang-$rowKirimb->saldo)>0){
              ...
              echo "<div class=box-body>
                      ...
                      <div class=form-group>
                        <label for=inputUserKaryawan class=col-sm-2 control-label>Jumlah</label>
                          <div class=col-sm-10>
                            <input type=number name=jumlahb[] placeholder = 'Jumlah Order = ".($rowKirimb->panjang-$rowKirimb->saldo)." Meter' max ='".($rowKirimb->panjang-$rowKirimb->saldo)."'required autofocus class=form-control>
...
<script>
  $("#Kirim").click(function(){
    ...
    $('input[name="harga[]"]').each( function(){ harga.push($(this).val()); });
    $('select[name="idBahan[]"]').each( function(){ bahan.push($(this).val()); });
    $('input[name="jumlahb[]"]').each( function(){ qtyb.push($(this).val()); });
    $('input[name="hargab[]"]').each( function(){ hargab.push($(this).val()); });
    $('input[name="tanggal"]').each( function(){ tanggal = $(this).val(); });
    $('input[name="supplier"]').each( function(){ supplier = $(this).val(); });
    $('input[name="noPO"]').each( function(){ noPO = $(this).val(); });
    $('input[name="noNota"]').each( function(){ noNota = $(this).val(); });
    $.ajax({
      type: "POST",
      url: "manage.php?act=insertpembelian",
      data: 'noNota=' + noNota +'&idSupplier='+ supplier +'&tanggal=' + tanggal,
      success: function(result) {
        for( i = 0 ;i < barang.length ; i++){
          $.ajax({
            type: "POST",
            url: "manage.php?act=insertpembelianbarang",
            data: 'noNota=' + noNota+ '&barang_id=' + barang[i]+ '&qty=' + qty[i]+ '&harga=' + harga[i]+ '&noPO='+noPO,
            success: function(result) {
              selesai++;
        ...
        for( i = 0 ;i < bahan.length ; i++){
          $.ajax({
            type: "POST",
            url: "manage.php?act=insertpembelianbahan",
            data: 'noNota=' + noNota+ '&barang_id=' + bahan[i]+ '&qty=' + qtyb[i]+ '&harga=' + hargab[i]+ '&noPO='+noPO,
            success: function(result) {
              selesai++;
              alert(selesai);
              if(selesai == bahan.length+barang.length){
